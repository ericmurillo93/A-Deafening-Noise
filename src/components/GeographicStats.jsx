  ...
  return <section className="rounded-3xl border border-zinc-800 bg-zinc-900 p-5 md:p-6">
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1.65fr)_minmax(220px,0.65fr)]">
      <div className="relative min-h-64 overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-950">
        ...
        <div className="absolute bottom-3 right-3 flex flex-col gap-1"><button onClick={resetMap} className="flex h-9 w-9 items-center justify-center rounded-xl border border-zinc-700 bg-zinc-900 text-zinc-300 shadow-lg transition hover:border-zinc-500 hover:text-white" aria-label="Reset map"><i className="fa-solid fa-house" aria-hidden="true" /></button><button onClick={() => setMapZoom((zoom) => Math.min(8, zoom * 1.35))} className="flex h-9 w-9 items-center justify-center rounded-xl border border-zinc-700 bg-zinc-900 text-zinc-300 shadow-lg transition hover:border-zinc-500 hover:text-white" aria-label="Zoom map in"><i className="fa-solid fa-plus" aria-hidden="true" /></button><button onClick={() => setMapZoom((zoom) => Math.max(1, zoom / 1.35))} className="flex h-9 w-9 items-center justify-center rounded-xl border border-zinc-700 bg-zinc-900 text-zinc-300 shadow-lg transition hover:border-zinc-500 hover:text-white" aria-label="Zoom map out"><i className="fa-solid fa-minus" aria-hidden="true" /></button></div>
